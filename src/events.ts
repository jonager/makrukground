...
export function bindDocument(s: State, onResize: () => void): cg.Unbind {
  ...
  // makrukground.resize if a user action has changed the bounds of the board.
  if (!('ResizeObserver' in window)) unbinds.push(unbindable(document.body, 'makrukground.resize', onResize))
